...
CREATE TABLE IF NOT EXISTS identificador_contrato (
    id INTEGER NOT NULL,
    numero_portabilidade DECIMAL(21) NOT NULL,
    data_insercao TIMESTAMP WITHOUT TIME ZONE NOT NULL,
    codigo_contrato_original CHAR(40) NOT NULL,
    cnpj_base_if_credora_original INTEGER NOT NULL,
...
CREATE TABLE IF NOT EXISTS cliente (
    ...
    PRIMARY KEY (id, numero_portabilidade, data_insercao),
    FOREIGN KEY (id, numero_portabilidade, data_insercao) REFERENCES solicitacao (id, numero_portabilidade, data_insercao),
    CONSTRAINT email_minimo_caracteres CHECK (length(email) >= 5)
);
...
CREATE TABLE IF NOT EXISTS proposta (
     ...
     quantidade_total_parcelas INTEGER NOT NULL,
...
CREATE TABLE endereco_carta_portabilidade (
    ...
    CONSTRAINT cidade_valido CHECK (length(cidade) >= 1)
);